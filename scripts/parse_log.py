...
if len(sys.argv) != 6:
    print("Requires five arguments: read-file-log.txt voltages-filename.csv temps-filename.csv pack_current-filename.csv pack_voltage-filename.csv")
    sys.exit(1)
...
def get_temps(parts):
    vals = []
    for part in parts[3:]:
        vals.append(int(part.strip())/10.0)
    return vals

def get_pack_current(parts):
    return parts[2]

def get_pack_voltage(parts):
    return parts[2]
...
for line in lines:
    ...
    if (not (parts[1]=='cvs' or parts[1]=='temps' or parts[1]=='pcurr' or parts[1]=='pvolt')): continue
    time = get_time(parts)
    if parts[1] == 'cvs':
        ...
        pack_voltage = get_pack_voltage(parts)
        line = str(time) + "," + str(pack_voltage)
        pack_voltage_lines.append(line)


print(temp_lines)
print(voltage_lines)
print(pack_current_lines)
print(pack_voltage_lines)
...
with open(pack_current_file, 'w') as f:
    ...
    for line in pack_current_lines:
        f.write(line + "\n")
with open(pack_voltage_file, 'w') as f:
    print("Writing pack voltage to file", pack_voltage_file)
    for line in pack_voltage_lines:
        f.write(line +"\n")
